...
- All `<video>` tags need `preload="none"` (STRATEGY §Phase 5 perf column) so nothing downloads until in view; pair with the matching `poster` JPG so there's no layout flash.
